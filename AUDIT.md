...
- The repository **`undivisible/unthinkclaw`** now 301-redirects to **`tschk/apollo`**. The pinned v0.1.1 asset URLs still resolve, so the formula still installs, but it ships an old build under a renamed project.
- Not bumped here: a bump requires verifying new asset names/SHAs against `tschk/apollo`. Recommended follow-up: bump and retest against the canonical repo, or remove the formula.

### `rx4` — audit script false-positive

- Formula pins **v0.6.5**; GitHub `releases/latest` currently points at **v0.6.3**, but tags **v0.6.4** and **v0.6.5** exist and the v0.6.5 source tarball resolves.
- `scripts/audit-formulas.sh` compares against `gh release view … latest`, so it would report `rx4` as stale incorrectly. Prefer comparing to the newest *tag* when pre-releases exist, or treat this as known.

## Safe scope

No invented checksums. No unthinkclaw bump. Docs + tk URL canonicalize only.
